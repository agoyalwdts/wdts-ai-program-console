/**
 * PATCH /api/cursor-prudence-alerts/:id
 *   Body: { "acknowledged": true } — sets acknowledgedAt (FinOps review).
 */
...
export async function PATCH(
  ...
  const now = new Date();
  const res = await prisma.cursorUsagePrudenceAlert.updateMany({
    where: { id, acknowledgedAt: null },
    data: { acknowledgedAt: now },
  });

  if (res.count === 0) {
    const exists = await prisma.cursorUsagePrudenceAlert.findUnique({
      where: { id },
