/**
 * Synthetic GatewayClient — reads from the dev Postgres (which is itself
 * populated by the deterministic synthetic seed in `prisma/seed.ts`).
 *
 * In v0.1 we rendered pages by querying Prisma directly from Server
 * Components. The synthetic client preserves that behaviour but routes the
 * reads through the GatewayClient interface so v0.2+ features (and the
 * eventual real client) consume the same shape.
 */
...
export const syntheticGatewayClient: GatewayClient = {
    ...
    const rows: ManagerQueueRow[] = reports.map((r) => {
      ...
      for (const key of PRODUCT_KEYS) {
        ...
        if (!lic || lic.capUsdMonth == null || lic.capUsdMonth === 0) {
          cap[key] = lic ? null : null; // null = no cap (not licensed or seat-priced)
        } else {
