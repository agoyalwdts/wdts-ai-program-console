...
describe("loadCodexEnterpriseSpendForF1", () => {
  it("prefers VendorDailySpend mirror when rows exist", async () => {
  ...
  it("falls back to live API when mirror is empty", async () => {
    mockFetchWorkspace.mockResolvedValue([
      {
    ...
    const prisma = {
      vendorDailySpend: { findMany: vi.fn().mockResolvedValue([]) },
  ...
  it("returns sync none when mirror empty and live fetch throws", async () => {
    ...
    expect(result.usedVendor).toBe(false);
  });
});
